...
// Function to get all users with only name and weekPoints
export const getAllUsersForLeaderboard = async () => {
    try {
        const users = await databases.listDocuments(
            ...
            0,   // No limit set
            0,   // Offset
            ['username', 'weekPoints']  // Attributes to retrieve
        );
...
// Function to get all leagues with only name and weekly-total-points
export const getAllLeaguesForLeaderboard = async () => {
    try {
        const leagues = await databases.listDocuments(
            ...
            0,   // No limit set
            0,   // Offset
            ['name', 'weekly-total-points', 'rank', "$id"]  // Attributes to retrieve
        );

        return leagues.documents.map(league => ({
            name: league.name,
            'weekly-total-points': league['weekly-total-points'],
            rank: league.rank,
            $id: league.$id
        }));
    } catch (error) {
...
export const createWeeklyLineup = async (picks, weekNumber) => {
    try {
        const currentUser = await getCurrentUser();
        if (!currentUser) throw new Error('User not found');
...
export const updatePick = async (pickTitle, potentialPoints, userId) => {
    try {
        console.log("1")
...
export const getPicksByIds = async (pickIds) => {
    try {
        ...
        // Loop through all pick IDs and fetch each pick document
...
export const getAllWeeklyLineups = async () => {
    try {
        ...
        const weeklyLineups = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.weeklyLineupCollectionId,
            [Query.equal('user', currentUser.$id)] // Filter by user.id
        );
...
export const createPick = async (pickTitle, potentialPoints, status, game, date, time, week) => {
    try {
...
export const removePickFromWeeklyLineup = async (pickId, userId, weekNumber, pts) => {
    try {
        ...
        if (lineups.documents.length > 0) {
            ...
            c//onst newTotalPotentialPoints = currentTotalPotentialPoints - pts;
            const updatedPicks = weeklyLineup.picks.filter(pick => pick.$id !== pickId);
...
export const updateWeeklyLineup = async (weekNumber, picks, pts) => {
    try {
        console.log('1')
        const currentUser = await getCurrentUser();
        if (!currentUser) throw new Error('User not found');
...
export const resetWeekPoints = async () => {
    try {
