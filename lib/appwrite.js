...
// Function to retrieve the current week number from the database
export const getCurrentWeekNum = async () => {
    try {
        ...
        if (result.documents.length > 0) {
            ...
            return currentWeekDocument.weekNum;  // Return the current week number
        } else {
...
export const leaveLeague = async (currentUser, currentLeague) => {
    try {
        // Remove the user from the league's users array
        const updatedUsers = currentLeague.users.filter((u) => u.$id !== currentUser.$id);

        // Update the league's users array
        const updatedLeague = await databases.updateDocument(
            appwriteConfig.databaseId,
            appwriteConfig.leagueCollectionId,
            currentLeague.$id,
        ...
        // Remove the league from the user's document
        const updatedUser = await databases.updateDocument(
            appwriteConfig.databaseId,
            appwriteConfig.userCollectionId,
            currentUser.$id,
