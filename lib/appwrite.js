...
export const createPick = async (pickTitle, potentialPoints, status) => {
    try {
...
export const updateWeeklyLineup = async (weekNumber, newPick) => {
    try {
        const currentUser = await getCurrentUser();
        if (!currentUser) throw new Error('User not found');
        ...
        let picks = weeklyLineup.documents[0].picks;
        if (picks) {
            picks.push(newPick.$id);
        } else {
            picks = [newPick.$id];
        }
        ...
        const updatedUser = await updateUserAttributes({ "weekly-lineup": [updatedLineup.$id] });

        return { updatedUser, updatedLineup };
