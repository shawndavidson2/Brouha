...
const calculateWeeklyPointsForAllUsers = async () => {
    try {
        ...
        for (const user of allUsers) {
            try {
                // Get user's weekly lineup for Week 2
                const weeklyLineup = await getUserWeeklyLineup(weekNum, user.$id);
                ...
                if (!picks || picks.length === 0) {
                    console.log(`User ${user.username} has no valid picks for Week ${weekNum}.`);
                    continue;
                }
                ...
                if (totalPoints !== user.weekPoints) {
                    console.log(`SOMETHING IS WRONG: User: ${user.username}, Week ${weekNum} Points: ${totalPoints}, On Appwrite: ${user.weekPoints}`);
                }
