...
const acquireLock = promisify(redisClient.setnx).bind(redisClient);
const releaseLock = promisify(redisClient.del).bind(redisClient);
const lockExpiry = 5000; // Lock expires after 5 seconds
  ...
  if (req.body["status"] === 'won') {
    for (const userId of req.body["users"]) {
      ...
      try {
        ...
        const lockAcquired = await acquireLock(lockKey, lockExpiry);
        if (!lockAcquired) {
