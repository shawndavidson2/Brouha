  ...
  if (status === 'won' || (status === 'lost')) {
    ...
    for (const userId of req.body["users"]) {
      ...
      try {
        ...
        if (plusMinus === "LOST") {
          log(`User ${user.username}: Pick lost, Added 1 to numPicksLost`)
          await releaseLock(userLockId);
          log(`Lock released for user ${userLockId}`);
          continue;
        }
        if (updatedUser) {
          log(`User ${user.username}: Added ${points} points. weekPoints is now: ${userWeekPoints} ... ${userNumPicksWon}-${userNumPicksLost}`);
        } else {
