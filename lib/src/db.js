...
export const acquireLockWithRetry = async (lockId, initialRetryInterval = 1500, maxRetries = 8) => {
    let attempts = 0;
...
export const getUserAttributes = async (userId) => {
    try {
        ...
        const { username, totalPoints, weekPoints, league, bestWeek, numPicksWon, numPicksLost } = user;

        return { username, totalPoints, weekPoints, league, bestWeek, numPicksWon, numPicksLost };
    } catch (error) {
